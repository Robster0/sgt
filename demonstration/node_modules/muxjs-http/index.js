...
class MuxJS 
{
    ...
    #HandleResponse(w, r, body) {
        try
        {
            ...
            for(let i = 0; i<this.#static.length; i++) 
                if(r.Url.substring(0, this.#static[i].root.length) === this.#static[i].root)
                    return w.SendFile(this.#static[i].path + r.Url)
    ...
    * @param {string} parent path to the parent folder, 
    * should only be used if the file calling "FileServer" is inside a subfolder relative to the desired statically served folder
    */
    FileServer(root, parent = module.parent.path) {
        try
        {
            ...
            this.#static.push({root, path: parent})
        }
        catch(err)
        {
            console.log(err);
